...
import static org.jclouds.dimensiondata.cloudcontrol.utils.DimensionDataCloudControllerUtils.manageResponse;
...
public class CleanupServer implements Function<String, Boolean> {
   ...
   protected final DimensionDataCloudControlApi api;
   private final Timeouts timeouts;

   @Inject
   public CleanupServer(DimensionDataCloudControlApi api, Timeouts timeouts) {
      this.api = api;
   ...
   public Boolean apply(final String serverId) {
      ...
      for (final NatRule natRule : natRulesToBeDeleted) {
         ...
         manageResponse(deleteNatRuleResponse, format("Cannot delete NAT rule for internalIp (%s) - externalIp %s created for server (%s). Rolling back ...", 
                 natRule.id(), 
                 natRule.internalIp(),
                 serverId));
